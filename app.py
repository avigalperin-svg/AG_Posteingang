...
async function runAll(){
  ...
  try{
    ...
    // Drive Posteingang - gestempeltes PDF hochladen
    lg('ti-brand-google-drive','var(--ac)','Speichere in Drive Posteingang...');setSP('sp4','run');
    const dr=await fetch('/drive/save',{method:'POST',headers:J,
      body:JSON.stringify({
        folder:'in',
        filename:`Posteingang_${nr}_${datum.replace(/\./g,'-')}.pdf`,
        pdf_b64:stPdfB64
      })});
    const dd=await dr.json();
    if(dd.ok){setSP('sp4','ok');lg('ti-brand-google-drive','var(--gr)','Posteingang in Drive gespeichert ✓');}
    else{setSP('sp4','fail');lg('ti-alert-triangle','var(--am)','Drive Posteingang: '+(dd.error||dd.msg));}
...
async function execDec(){
  ...
  try{
    if(curD==='wv'){
      ...
      // Drive Postausgang - gestempeltes PDF speichern
      lg('ti-brand-google-drive','var(--ac)','Speichere Ausgang in Drive...');
      const adr=await fetch('/drive/save',{method:'POST',headers:J,
        body:JSON.stringify({
          folder:'out',
          filename:`Ausgang_${nr}_${datum.replace(/\./g,'-')}_${to.replace('@','_at_')}.pdf`,
          pdf_b64:stPdfB64
        })});
      const add=await adr.json();
      if(add.ok)lg('ti-brand-google-drive','var(--gr)','Ausgang in Drive gespeichert ✓');
      else lg('ti-alert-triangle','var(--am)','Drive Ausgang: '+(add.error||add.msg));
      await fetch('/tg',{method:'POST',headers:J,body:JSON.stringify({
        ...
        if self.path=='/config/save':
            ...
            try:
                send_email(cfg,
                    b.get('to',''),
                    b.get('cc',''),
                    b.get('subject','Antwort – AG Posteingang'),
                    b.get('body',''),
                    b.get('pdf_b64'),
                    b.get('pdf_name'))
