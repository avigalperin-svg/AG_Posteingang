...
def gdrive_list_new_files(cfg, since_iso, folder_id=None):
    """Listet neue Dateien im Drive-Ordner seit einem Zeitpunkt"""
...
def split_pdf_document(pdf_b64, filename, cfg):
    """
    Versucht ein Sammeldokument zu trennen.
    Einfache Heuristik: Seiten mit wenig Inhalt = Trennseiten
    Für echte Trennung: Claude analysiert das Dokument
    """
    try:
        pdf_bytes = base64.b64decode(pdf_b64)
        ...
        if total_pages <= 1:
            return {'ok': True, 'parts': 1, 'filenames': [filename]}

        # Einfache Trennung: alle 3 Seiten als neues Dokument (Heuristik)
        # In Praxis: KI-basierte Erkennung von Dokumentenenden
        parts = []
        chunk_size = max(1, total_pages // 2) if total_pages <= 6 else 3
        ...
        for i in range(0, total_pages, chunk_size):
            ...
            part_name = f"{base_name}_Teil{len(parts)+1}.pdf"
            filenames.append(part_name)

            # In Drive Posteingang speichern
            if cfg.get('gdrive_in'):
                gdrive_upload_pdf(cfg, cfg['gdrive_in'].strip().split('?')[0],
                                  part_name, part_b64)
            parts.append({'name': part_name, 'pdf_b64': part_b64})

        return {'ok': True, 'parts': len(parts), 'filenames': filenames}
    except Exception as e:
        return {'ok': False, 'error': str(e), 'parts': 1}
...
async function splitDoc(){
  ...
  const r=await fetch('/doc/split',{method:'POST',headers:J,body:JSON.stringify({pdf_b64:b64,filename:f.name})});
  const d=await r.json();
  if(d.ok&&d.parts>1){
    lg('ti-files','var(--gr)',`${d.parts} Dokumente erkannt und getrennt ✓`);
    alert(`${d.parts} Dokumente erkannt. Bitte die Dokumente einzeln bearbeiten.\n\nSie wurden im Drive-Posteingang-Ordner gespeichert als:\n${d.filenames?.join('\n')||''}`);
    $('multi-doc').style.display='none';
  } else {
...
async function runAll(){
  ...
  try{
    ...
    if(sd.ok){
      if(sd.drive_ok){setSP('sp4','ok');lg('ti-brand-google-drive','var(--gr)','Posteingang in Drive ✓');}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path=='/config/save':
            ...
            # Prüft Drive-Ordner auf neue Dokumente
            cfg=load_config()
            since=b.get('since','2020-01-01T00:00:00Z')
            try:
                files=gdrive_list_new_files(cfg,since)
                result_files=[]
                for f in files[:5]:  # max 5 neue Dateien auf einmal
