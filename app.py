    ...
    def do_POST(self):
        ...
        if self.path=='/config/save':
            ...
            try:
                ...
                # 2. Welches Konto hat den Token?
                req=urllib.request.Request(
                    'https://www.googleapis.com/oauth2/v3/userinfo',
                    headers={'Authorization':f'Bearer {token}'}
                )
                userinfo=json.loads(urllib.request.urlopen(req,timeout=10).read())
                result['konto']=userinfo.get('email','unbekannt')

                # 3. Drive-Zugriff testen - eigene Dateien auflisten
                req2=urllib.request.Request(
                    'https://www.googleapis.com/drive/v3/files?pageSize=3&fields=files(id,name)',
                    headers={'Authorization':f'Bearer {token}'}
                )
                files=json.loads(urllib.request.urlopen(req2,timeout=10).read())
                result['drive_zugriff']='OK'
                result['dateien_gefunden']=len(files.get('files',[]))

                # 4. Ordner direkt prüfen
