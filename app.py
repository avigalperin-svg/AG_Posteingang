...
def login_html(error=False):
    err = '<p style="color:#A32D2D;margin-top:10px;font-size:13px">Falscher Zugangscode – bitte nochmal versuchen</p>' if error else ''
...
body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;background:#F0EFE9;min-height:100vh;display:flex;align-items:center;justify-content:center}}
.box{{background:#fff;border-radius:12px;padding:40px 36px;width:340px;box-shadow:0 4px 24px rgba(0,0,0,0.10);text-align:center}}
.logo{{width:52px;height:52px;background:#185FA5;border-radius:12px;display:flex;align-items:center;justify-content:center;margin:0 auto 18px;font-size:26px}}
h1{{font-size:20px;font-weight:600;margin-bottom:6px;color:#1C1C1A}}
p{{font-size:13px;color:#6B6A66;margin-bottom:24px}}
input{{width:100%;padding:11px 13px;border:1px solid rgba(0,0,0,0.18);border-radius:8px;font-size:15px;font-family:inherit;outline:none;margin-bottom:14px;background:#F4F3EE;text-align:center}}
input:focus{{border-color:#185FA5;background:#fff}}
button{{width:100%;padding:12px;background:#185FA5;color:#fff;border:none;border-radius:8px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit}}
...
<div class="box">
  ...
  <form method="POST" action="/auth/login">
    <input type="password" name="password" placeholder="Zugangscode eingeben" autofocus/>
...
class H(BaseHTTPRequestHandler):
    ...
    def session_ok(self):
        # Cookie-basierter Session-Check
        cookie_header = self.headers.get('Cookie', '')
        for part in cookie_header.split(';'):
            part = part.strip()
            if part.startswith('sela_token='):
                token = part[len('sela_token='):]
                if check_session(token):
                    return True
        # Fallback: X-Session Header (für API-Calls vom Browser-JS)
    ...
    def do_POST(self):
        # Form-Login (application/x-www-form-urlencoded)
        if self.path == '/auth/login':
            ...
            if pw == APP_PASSWORD:
                ...
                self.send_response(302)
                self.send_header('Location', '/app')
                self.send_header('Set-Cookie', f'sela_token={token}; Path=/; HttpOnly; SameSite=Lax')
                self.cors()
                self.end_headers()
            else:
                self.html(login_html(error=True))
            return
