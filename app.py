...
async function openArchiveDoc(idx){
  ...
  // Vorschau-Panel direkt aktualisieren — KEIN Tab-Wechsel
  ...
  try{
    ...
    if(d.ok&&d.pdf_b64){
      ...
      $('prev-foot').innerHTML=`
        <span style="color:var(--gr)"><i class="ti ti-brand-google-drive"></i> Drive</span>
        <span style="font-weight:500">${d.filename}</span>
        <span style="color:var(--tx2)">${e.datum||''} · ${e.absender||'–'}</span>
        ${e.betrag?`<span style="color:var(--am)">💶 ${e.betrag}</span>`:''}`;
      lg('ti-brand-google-drive','var(--gr)',`Archiv-Vorschau: ${d.filename} ✓`);
    } else {
