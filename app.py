...
def send_email(cfg, to, cc, subject, body_text, pdf_b64=None, pdf_name=None):
    ...
    msg = MIMEMultipart()
    msg['From']    = f"{cfg.get('sender_name','Sela Holding')} <{user}>"
    msg['To']      = to
    if cc: msg['Cc'] = cc
    msg['Subject'] = subject
...
# ─── GOOGLE DRIVE UPLOAD (HTTPS Port 443) ────────────────────────────────────
...
.panel-left{width:500px;min-width:300px;flex-shrink:0;overflow-y:auto;
...
async function testMail(){
  ...
  const r=await fetch('/mail',{method:'POST',headers:J,
    body:JSON.stringify({to,subject:'Test – Sela Holding',body:'Verbindungstest OK.'})});
  const d=await r.json();
...
async function testDrive(){
  $('cfgmsg').innerHTML='<span class="bdg binfo"><i class="ti ti-loader"></i> Teste Drive...</span>';
  const r=await fetch('/drive/save',{method:'POST',headers:J,
    body:JSON.stringify({folder:'in',filename:'Drive_Test.txt',text:`Test ${tod()} ${now()}`})});
  const d=await r.json();
  $('cfgmsg').innerHTML=d.ok?'<span class="bdg bok"><i class="ti ti-check"></i> Drive OK!</span>':`<span class="bdg berr">Drive: ${d.error||d.msg}</span>`;
...
async function runAll(){
  ...
  try{
    ...
    else if(sd.drive_msg&&sd.drive_msg!=='Drive nicht konfiguriert'){setSP('sp4','fail');lg('ti-alert-triangle','var(--am)','Drive: '+sd.drive_msg);}
  ...
  try{
    if(curD==='wv'){
      ...
      try{
        ...
        if(ed.drive_ok)lg('ti-brand-google-drive','var(--gr)','Ausgang in Drive gespeichert ✓');
        else if(ed.drive_msg&&ed.drive_msg!=='Drive nicht konfiguriert')lg('ti-alert-triangle','var(--am)','Drive Ausgang: '+ed.drive_msg);
        ...
        if self.path=='/config/save':
            ...
            try:
                send_email(cfg,
                    ...
                    b.get('pdf_b64'),
                    b.get('pdf_name'))
                # Nach erfolgreichem Versand: Ausgang in Drive speichern
                drive_ok=False; drive_msg='Drive nicht konfiguriert'
                if cfg.get('gdrive_out') and b.get('pdf_b64'):
                    n=datetime.datetime.now()
                    dat=n.strftime('%d%m%Y')
                    fname=f"Ausgang_{b.get('nr','')or dat}_{dat}.pdf"
                    drive_ok,drive_msg=gdrive_upload_pdf(cfg,cfg['gdrive_out'],fname,b['pdf_b64'])
                self.send_json({'ok':True,'drive_ok':drive_ok,'drive_msg':str(drive_msg)})
            except Exception as e:
                self.send_json({'ok':False,'error':str(e)})

        elif self.path=='/drive/save':
            # Manueller Drive-Upload (z.B. für Wiedervorlage-Protokoll)
            cfg=load_config()
            folder=b.get('folder','in')
            folder_id=cfg.get('gdrive_in' if folder=='in' else 'gdrive_out','')
