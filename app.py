...
<div class="card">
  ...
  <div class="g2">
    ...
    <div style="background:var(--acl);border-radius:var(--rs);padding:10px 13px;font-size:12px;color:var(--acd);margin-top:4px">
      <i class="ti ti-info-circle"></i> <b>Wie bekommst du Client ID, Secret & Refresh Token?</b><br>
      Lade die Datei <code>get_token.py</code> herunter → lokal ausführen → Werte hier eintragen.
      <a href="#" onclick="showTokenHelp()" style="color:var(--ac);font-weight:500"> Anleitung anzeigen</a>
    </div>
    <div id="token-help" style="display:none;margin-top:10px;background:var(--sf2);border-radius:var(--rs);padding:12px;font-size:12px;line-height:2;border:.5px solid var(--bd)">
      <b>Schritt 1</b> – Google Cloud Console: <code>console.cloud.google.com</code><br>
      <b>Schritt 2</b> – Neues Projekt erstellen → „Gmail API" aktivieren<br>
      <b>Schritt 3</b> – OAuth2 Zugangsdaten → Desktop App → Client ID &amp; Secret kopieren<br>
      <b>Schritt 4</b> – <code>get_token.py</code> lokal ausführen → Browser öffnet sich → Anmelden<br>
      <b>Schritt 5</b> – Refresh Token erscheint im Terminal → hier eintragen
    </div>
...
async function saveCfg(){
  const body={
    ...
    gmail_client_id:$('c-ci').value.trim(),
    gmail_client_secret:$('c-cs').value.trim(),
