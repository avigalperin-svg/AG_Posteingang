...
.modal-box{background:var(--sf);border-radius:var(--r);width:92vw;max-width:980px;
  height:88vh;display:flex;flex-direction:column;overflow:hidden;
  box-shadow:0 8px 40px rgba(0,0,0,.25)}
.modal-hdr{padding:14px 18px;border-bottom:.5px solid var(--bd);display:flex;
  align-items:center;justify-content:space-between;flex-shrink:0}
.modal-hdr h2{font-size:15px;font-weight:600;display:flex;align-items:center;gap:8px}
.modal-body{flex:1;overflow:hidden;display:flex;gap:0}
/* Seitenleiste: Thumbnails */
.split-pages{width:200px;flex-shrink:0;overflow-y:auto;border-right:.5px solid var(--bd);
  padding:10px 8px;background:var(--sf2)}
.split-page-item{position:relative;margin-bottom:6px;cursor:pointer;border-radius:var(--rs);
  overflow:hidden;border:2px solid transparent;transition:border-color .15s}
.split-page-item:hover{border-color:var(--ac)}
.split-page-item.selected{border-color:var(--ac);background:var(--acl)}
.split-page-item canvas{width:100%;display:block;border-radius:4px}
.page-num{position:absolute;top:4px;left:4px;background:rgba(0,0,0,.55);color:#fff;
  font-size:10px;font-weight:600;padding:1px 5px;border-radius:3px}
.split-marker{position:absolute;bottom:0;left:0;right:0;height:3px;background:var(--re)}
.split-marker-label{position:absolute;bottom:4px;right:4px;background:var(--re);color:#fff;
  font-size:9px;font-weight:600;padding:1px 5px;border-radius:3px}
/* Hauptvorschau im Modal */
.split-preview{flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px}
.split-doc-group{border:1.5px solid var(--bd2);border-radius:var(--rs);overflow:hidden}
.split-doc-header{background:var(--sf2);padding:8px 12px;display:flex;align-items:center;
  justify-content:space-between;font-size:12px;font-weight:600;border-bottom:.5px solid var(--bd)}
.split-doc-header.active{background:var(--acl);color:var(--acd)}
.split-pages-preview{padding:8px;display:flex;gap:6px;flex-wrap:wrap}
.split-page-thumb{width:80px;height:110px;background:white;border:.5px solid var(--bd);
  border-radius:4px;display:flex;align-items:center;justify-content:center;
  font-size:10px;color:var(--tx3);overflow:hidden;position:relative}
.split-page-thumb canvas{width:100%;height:100%;object-fit:contain}
/* Trennlinie */
.split-divider{border:none;border-top:2px dashed var(--re);margin:6px 0;
  position:relative;cursor:pointer}
.split-add-break{background:var(--rel);color:var(--re);border:.5px solid var(--re);
  border-radius:20px;padding:2px 10px;font-size:10px;font-weight:600;cursor:pointer;
...
<div class="card">
  ...
  <div class="multi-doc" id="multi-doc">
    <i class="ti ti-files"></i> <b>Mehrseitiges Dokument</b> — enthält dieses PDF mehrere Schreiben?
    <div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap">
      <button class="btn bs" style="font-size:11px" onclick="openSplitModal()">
        <i class="ti ti-scissors"></i> Vorschau & Trennen
      </button>
      <button class="btn bs" style="font-size:11px" onclick="$('multi-doc').style.display='none'">
        <i class="ti ti-x"></i> Kein Sammeldokument
      </button>
    </div>
  </div>
</div>
...
<div class="card" id="c2" style="display:none">
  ...
  <div class="idx-box" id="idx-box">
    <div class="idx-title"><i class="ti ti-forms"></i> Pflichtfelder & Indexierung
      <span id="idx-valid-badge" style="margin-left:8px"></span>
    </div>
    ...
    <div class="g2" style="margin-bottom:10px">
      ...
      <!-- Empfängertyp -->
      <div class="fld">
        <label>Empfänger-Typ <span class="req">*</span></label>
        <select id="idx-empf-typ" onchange="updateIdxFields()">
      ...
      <!-- Empfänger Name -->
      <div class="fld">
        <label>Empfänger (Name/Firma) <span class="req">*</span></label>
        <input type="text" id="idx-empf-name" placeholder="Name oder Firma..."/>
      </div>

      <!-- Absender -->
      <div class="fld full">
        <label>Absender <span class="req">*</span></label>
        <input type="text" id="idx-absender" placeholder="Absender des Schreibens..."/>
      </div>

      <!-- Dokument-Datum -->
      <div class="fld">
        <label>Dokumentdatum <span class="req">*</span></label>
        <input type="text" id="idx-dok-datum" placeholder="TT.MM.JJJJ"/>
      </div>

      <!-- Eingangsdatum -->
      <div class="fld">
        <label>Eingangsdatum <span class="req">*</span></label>
        <input type="text" id="idx-eing-datum" placeholder="TT.MM.JJJJ"/>
      </div>

      <!-- Dokumentenart -->
      <div class="fld">
        <label>Dokumentenart <span class="req">*</span></label>
        <select id="idx-dtype" onchange="updateIdxFields()">
          <option value="">-- Bitte wählen --</option>
        </select>
      </div>
      ...
      <div class="fld">
        <label>Betreff</label>
        <input type="text" id="idx-betreff" placeholder="Kurzbeschreibung..."/>
      </div>

    </div>

    <!-- RECHNUNGSFELDER (nur bei Dokumentenart = Rechnung) -->
    <div id="rechnung-felder" style="display:none;background:var(--aml);border:.5px solid var(--am);
      border-radius:var(--rs);padding:10px;margin-bottom:10px">
      <div style="font-size:11px;font-weight:600;color:var(--am);text-transform:uppercase;
        letter-spacing:.04em;margin-bottom:8px">
        <i class="ti ti-receipt"></i> Rechnungsfelder (Pflicht)
      </div>
      <div class="g2">

        <!-- Belegnummer -->
        <div class="fld">
          <label>Belegnummer <span class="req">*</span></label>
          <input type="text" id="idx-belegnr" placeholder="Rechnungs-Nr..."/>
        </div>

        <!-- Rechnungsbetrag netto -->
        <div class="fld">
          <label>Rechnungsbetrag (Netto) <span class="req">*</span></label>
          <input type="text" id="idx-betrag-netto" placeholder="0,00" oninput="berechneSkonto()"/>
        </div>

        <!-- Zahlungsziel -->
        <div class="fld">
          <label>Zahlungsziel <span class="req">*</span></label>
          <input type="text" id="idx-zahlungsziel" placeholder="TT.MM.JJJJ"/>
        </div>

        <!-- Skontosatz -->
        <div class="fld">
          <label>Skontosatz (%)</label>
          <input type="text" id="idx-skontosatz" placeholder="z.B. 2" oninput="berechneSkonto()"/>
        </div>

        <!-- Skontofähiger Betrag -->
        <div class="fld">
          <label>Skontofähiger Betrag
            <span style="font-weight:400;font-size:10px">(leer = wird berechnet)</span>
          </label>
          <input type="text" id="idx-skonto-basis" placeholder="0,00" oninput="berechneSkonto()"/>
        </div>

        <!-- Skontobetrag (auto) -->
        <div class="fld">
          <label>Skontobetrag
            <span style="font-weight:400;font-size:10px">(auto berechnet)</span>
          </label>
          <input type="text" id="idx-skontobetrag" placeholder="0,00" readonly
            style="background:var(--sf2);color:var(--gr);font-weight:600"/>
        </div>

        <!-- Zahlungsziel bei Skonto -->
        <div class="fld">
          <label>Zahlungsziel bei Skonto</label>
          <input type="text" id="idx-skonto-frist" placeholder="TT.MM.JJJJ"/>
        </div>

        <!-- Zu zahlender Betrag -->
        <div class="fld">
          <label>Zu zahlender Betrag (mit Skonto)
            <span style="font-weight:400;font-size:10px">(auto)</span>
          </label>
          <input type="text" id="idx-zahlung-skonto" placeholder="0,00" readonly
            style="background:var(--sf2);color:var(--ac);font-weight:600"/>
        </div>

      </div>

      <!-- Skonto-Info Box -->
      <div id="skonto-info" style="display:none;margin-top:6px;padding:6px 10px;
        background:var(--grl);border-radius:var(--rs);font-size:11px;color:var(--gr)">
      </div>
    </div>
    ...
    <div class="g2">
      ...
      <div>
        <div class="idx-label">ANTWORTSPRACHE</div>
        <div class="lang-row" id="lang-row" style="flex-wrap:wrap;gap:4px;margin-top:4px">
          <span id="lang-detected" style="font-size:11px;color:var(--tx2);width:100%"></span>
          <button class="lang-btn sel" id="lang-de" onclick="setLang('de')">🇩🇪 DE</button>
          ...
          <button class="lang-btn" id="lang-orig" onclick="setLang('orig')">📄 Orig</button>
        </div>
      </div>
    </div>

    <!-- Validierungs-Hinweis -->
    <div id="idx-errors" style="display:none;margin-top:8px;padding:8px 10px;
      background:var(--rel);border-radius:var(--rs);font-size:11px;color:var(--re)"></div>

    <!-- Speichern Button -->
    <div class="brow" style="margin-top:10px">
      <button class="btn bp" onclick="saveIdxFields()" style="font-size:12px">
        <i class="ti ti-device-floppy"></i> Felder speichern & validieren
      </button>
    </div>
  </div>
...
<div class="card">
  ...
  <div class="g2" style="margin-bottom:10px">
    <div class="fld full">
      <label>Suche</label>
      <input type="text" id="arch-q" placeholder="Absender, Betreff, Betrag, Tag, Label..." onkeyup="searchArch()"/>
    </div>
...
<div class="modal-overlay" id="split-modal">
  <div class="modal-box">
    <div class="modal-hdr">
      <h2><i class="ti ti-scissors" style="color:var(--ac)"></i> Dokumente trennen
        <span id="split-info" style="font-size:12px;font-weight:400;color:var(--tx2)"></span>
      </h2>
      <div style="display:flex;gap:8px;align-items:center">
        <span style="font-size:12px;color:var(--tx2)">
          <span id="split-doc-count" style="font-weight:600;color:var(--ac)">1</span> Dokument(e) definiert
        </span>
        <button class="btn bp" id="btn-split-confirm" onclick="confirmSplit()">
          <i class="ti ti-check"></i> Trennung bestätigen & in Warteschlange
        </button>
        <button class="btn bs" onclick="closeSplitModal()">
          <i class="ti ti-x"></i> Abbrechen
        </button>
      </div>
    </div>
    <div class="modal-body">
      <!-- Links: Seiten-Thumbnails -->
      <div class="split-pages" id="split-pages-list">
        <div style="font-size:10px;font-weight:600;color:var(--tx2);text-transform:uppercase;
          letter-spacing:.04em;margin-bottom:8px">Seiten</div>
      </div>
      <!-- Rechts: Dokument-Gruppen -->
      <div class="split-preview" id="split-preview-area">
        <div style="font-size:12px;color:var(--tx2);padding:10px 0">
          <i class="ti ti-info-circle"></i>
          Klicke zwischen zwei Seiten auf <b>„Hier trennen"</b> um ein neues Dokument zu beginnen.
          Oder klicke auf eine bestehende Trennlinie um sie zu entfernen.
        </div>
        <div id="split-groups"></div>
      </div>
    </div>
  </div>
</div>
...
// ═══ DOKUMENTENARTEN ════
// ═══ SICHERE BASE64 HELFER (kein Stack Overflow bei großen Dateien) ════
...
async function processNext(){
  ...
  // Reset ohne Queue zu leeren
  resetAll();
  // Datei direkt in State laden — KEIN addFiles() um onchange-Loop zu vermeiden
  const mime=next.mime||'application/pdf';
  const bytes=b64ToBytes(next.b64);
  const blob=new Blob([bytes],{type:mime});
  const file=new File([blob],next.name,{type:mime});
  // Direkt in files array laden ohne Event
  files=[file];
  renderFiles();
  $('c2').style.display='block';
  // Vorschau zeigen
  origB64=next.b64;origMime=mime;
  showPreview(next.b64,mime,next.name,false);
  // Automatisch analysieren nach kurzer Pause
  await new Promise(r=>setTimeout(r,300));
  await runAll();
}

// ═══ SPLIT MODAL ════
let splitBreaks=[]; // Seitennummern NACH denen ein neues Dokument beginnt (0-basiert)
...
async function openSplitModal(){
  ...
  splitPdfB64=await toB64(f);
  splitBreaks=[];

  // Seitenanzahl ermitteln via pdf-lib
  const {PDFDocument}=PDFLib;
  let pageCount=1;
  try{
    const bytes=b64ToBytes(splitPdfB64);
  ...
  if(pageCount<=1){
    alert('Dieses PDF hat nur 1 Seite – keine Trennung möglich.');
    return;
  }

  renderSplitModal();
  $('split-modal').classList.add('open');
  lg('ti-scissors','var(--ac)',`Split-Vorschau geöffnet: ${pageCount} Seiten`);
}
...
function renderSplitModal(){
  $('split-info').textContent=`· ${splitPageCount} Seiten`;
  renderSplitGroups();
  renderSplitPageList();
}

function renderSplitGroups(){
  // Dokument-Gruppen berechnen
  const groups=getSplitGroups();
  $('split-doc-count').textContent=groups.length;

  let html='';
  groups.forEach((group,gi)=>{
    const isLast=gi===groups.length-1;
    html+=`
      <div class="split-doc-group">
        <div class="split-doc-header ${gi===0?'active':''}">
          <span><i class="ti ti-file-description"></i> Dokument ${gi+1}
            <span style="font-weight:400;color:var(--tx2);margin-left:6px">
              Seite${group.length>1?'n':''} ${group[0]+1}${group.length>1?' – '+(group[group.length-1]+1):''}
              (${group.length} Seite${group.length>1?'n':''})
            </span>
          </span>
          ${gi>0?`<button onclick="removeBreak(${group[0]-1})" style="background:none;border:none;
            cursor:pointer;color:var(--re);font-size:11px;display:flex;align-items:center;gap:3px">
            <i class="ti ti-trash"></i> Trennlinie entfernen</button>`:''}
        </div>
        <div class="split-pages-preview">
          ${group.map(p=>`
            <div class="split-page-thumb" title="Seite ${p+1}">
              <canvas id="thumb-${p}" width="80" height="110"></canvas>
              <div style="position:absolute;top:2px;left:2px;background:rgba(0,0,0,.5);
                color:#fff;font-size:9px;padding:1px 4px;border-radius:2px">${p+1}</div>
            </div>`).join('')}
        </div>
      </div>`;

    // Trenn-Button zwischen Gruppen
    if(!isLast){
      const breakAfter=group[group.length-1];
      html+=`
        <div style="text-align:center;padding:4px 0;position:relative">
          <hr style="border:none;border-top:2px dashed var(--re);margin:0">
          <span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
            background:var(--re);color:#fff;font-size:10px;font-weight:600;
            padding:2px 10px;border-radius:20px;white-space:nowrap">
            ✂ Trennung nach Seite ${breakAfter+1}
            <button onclick="removeBreak(${breakAfter})" style="background:none;border:none;
              color:#fff;cursor:pointer;font-size:10px;margin-left:4px;opacity:.8">✕</button>
          </span>
        </div>`;
    }
  });

  // Am Ende: Trenn-Button für jede Seite
  html+=`
    <div style="margin-top:12px;padding:10px;background:var(--sf2);border-radius:var(--rs)">
      <div style="font-size:11px;font-weight:600;color:var(--tx2);margin-bottom:8px;text-transform:uppercase;letter-spacing:.04em">
        Trennung hinzufügen nach Seite:
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:5px">
        ${Array.from({length:splitPageCount-1},(_,i)=>`
          <button onclick="addBreak(${i})"
            style="padding:3px 10px;border-radius:20px;font-size:11px;font-weight:500;
              cursor:pointer;font-family:inherit;
              background:${splitBreaks.includes(i)?'var(--rel)':'var(--acl)'};
              color:${splitBreaks.includes(i)?'var(--re)':'var(--acd)'};
              border:.5px solid ${splitBreaks.includes(i)?'var(--re)':'var(--ac)'}">
            ${splitBreaks.includes(i)?'✕ Seite '+(i+1):'+ Nach Seite '+(i+1)}
          </button>`).join('')}
      </div>
    </div>`;

  $('split-groups').innerHTML=html;
...
function renderSplitPageList(){
  $('split-pages-list').innerHTML=`
    <div style="font-size:10px;font-weight:600;color:var(--tx2);text-transform:uppercase;
      letter-spacing:.04em;margin-bottom:8px">Seiten</div>
    ${Array.from({length:splitPageCount},(_,i)=>`
      <div class="split-page-item ${splitBreaks.includes(i-1)?'selected':''}"
        style="position:relative">
        <canvas id="sidebar-thumb-${i}" width="180" height="250"
          style="width:100%;background:#f5f5f5;border-radius:4px;min-height:60px"></canvas>
        <div class="page-num">${i+1}</div>
        ${splitBreaks.includes(i)?`<div class="split-marker"></div>
          <div class="split-marker-label">Trennung</div>`:''}
      </div>
      ${i<splitPageCount-1?`
        <div style="text-align:center;margin:2px 0">
          <button onclick="toggleBreak(${i})" style="background:none;border:none;cursor:pointer;
            color:${splitBreaks.includes(i)?'var(--re)':'var(--tx3)'};font-size:10px;
            padding:1px 6px;border-radius:3px;width:100%;text-align:center;
            background:${splitBreaks.includes(i)?'var(--rel)':'transparent'}">
            ${splitBreaks.includes(i)?'✕ Trennung entfernen':'+ Hier trennen'}
          </button>
        </div>`:''}
    `).join('')}`;
}
...
async function renderThumbnails(){
  // Einfache Thumbnail-Darstellung mit Seitennummern (ohne externe PDF-Renderer)
  // Zeigt Seiten als nummerierte Platzhalter
  for(let i=0;i<splitPageCount;i++){
    ['thumb-'+i,'sidebar-thumb-'+i].forEach(id=>{
      ...
      ctx.font=`bold ${canvas.width>100?18:14}px sans-serif`;
      ctx.textAlign='center';
      ctx.fillText(`Seite`,canvas.width/2,canvas.height/2-10);
      ctx.fillText(`${i+1}`,canvas.width/2,canvas.height/2+15);
      ctx.strokeStyle='#ddd';
      ctx.lineWidth=1;
...
function getSplitGroups(){
  const groups=[];
  let start=0;
  const breaks=splitBreaks.slice().sort((a,b)=>a-b);
  for(const br of breaks){
    const pages=Array.from({length:br-start+1},(_,i)=>start+i);
    groups.push(pages);
    start=br+1;
  }
  // Letzte Gruppe
  const lastPages=Array.from({length:splitPageCount-start},(_,i)=>start+i);
  if(lastPages.length>0)groups.push(lastPages);
  return groups;
}

function addBreak(pageIndex){
  if(!splitBreaks.includes(pageIndex)){
    splitBreaks.push(pageIndex);
    splitBreaks.sort((a,b)=>a-b);
  }
  renderSplitGroups();
  renderSplitPageList();
}
function removeBreak(pageIndex){
  splitBreaks=splitBreaks.filter(b=>b!==pageIndex);
  renderSplitGroups();
  renderSplitPageList();
}
function toggleBreak(pageIndex){
  if(splitBreaks.includes(pageIndex))removeBreak(pageIndex);
  else addBreak(pageIndex);
}

async function confirmSplit(){
  const groups=getSplitGroups();
  if(groups.length<=1){
    alert('Bitte mindestens eine Trennlinie setzen um das Dokument zu teilen.');
    return;
  }
  closeSplitModal();
  lg('ti-scissors','var(--gr)',`Trenne in ${groups.length} Dokumente...`);

  // PDF aufteilen via pdf-lib im Browser
  const {PDFDocument}=PDFLib;
  const bytes=b64ToBytes(splitPdfB64);
  ...
  for(let gi=0;gi<groups.length;gi++){
    ...
    const b64=bytesToB64(new Uint8Array(pdfBytes));
    const name=`${baseName}_Dok${gi+1}_von_${groups.length}.pdf`;
    parts.push({name,b64,mime:'application/pdf'});
  }

  lg('ti-files','var(--gr)',`${parts.length} Dokumente erstellt`);
  ...
  if(parts.length>0){
    addToQueue(parts);
    lg('ti-list-numbers','var(--ac)',`${parts.length} weitere Dokumente in Warteschlange`);
  }

  // Erstes direkt laden — ohne addFiles() um Loop zu vermeiden
  resetAll();
  const firstBytes=b64ToBytes(first.b64);
  const blob=new Blob([firstBytes],{type:'application/pdf'});
  const file=new File([blob],first.name,{type:'application/pdf'});
  files=[file];
  renderFiles();
  $('c2').style.display='block';
  origB64=first.b64;origMime='application/pdf';
  showPreview(first.b64,'application/pdf',first.name,false);
  lg('ti-file','var(--ac)',`Bearbeite zuerst: ${first.name} (${parts.length+1} von ${groups.length})`);
}
...
// ═══ INDEXIERUNG ════
// ═══ PFLICHTFELDER & SKONTO-BERECHNUNG ════
function updateIdxFields(){
  // Rechnungsfelder ein/ausblenden
  const dtype=($('idx-dtype').value||'').toLowerCase();
  const isRechnung=dtype==='rechnung';
  $('rechnung-felder').style.display=isRechnung?'block':'none';
  // Dokumentenart-Select befüllen
  const sel=$('idx-dtype');
...
function berechneSkonto(){
  const netto=parseBetrag($('idx-betrag-netto').value);
  const satz=parseBetrag($('idx-skontosatz').value);
  const basis=parseBetrag($('idx-skonto-basis').value);

  if(satz>0){
    // Skontofähige Basis: entweder eingetragen oder = Nettobetrag
    const skontoBasis=basis>0?basis:netto;
    const skontoBetrag=skontoBasis*(satz/100);
    const zahlungMitSkonto=netto-skontoBetrag;

    if(skontoBasis>0){
      $('idx-skontobetrag').value=formatBetrag(skontoBetrag);
      $('idx-zahlung-skonto').value=formatBetrag(zahlungMitSkonto);
      // Falls Basis nicht manuell eingetragen, auto-setzen
      if(!basis)$('idx-skonto-basis').value=formatBetrag(skontoBasis);

      $('skonto-info').style.display='block';
      $('skonto-info').innerHTML=
        `<i class="ti ti-discount"></i> Skonto ${satz}% auf ${formatBetrag(skontoBasis)} € = `+
        `<b>${formatBetrag(skontoBetrag)} € Ersparnis</b> · `+
        `Zu zahlen bei Skonto: <b>${formatBetrag(zahlungMitSkonto)} €</b>`;
    }
  } else {
    $('idx-skontobetrag').value='';
    $('idx-zahlung-skonto').value='';
    $('skonto-info').style.display='none';
  }
  validateIdxFields();
}

function validateIdxFields(){
  const errors=[];
  const dtype=($('idx-dtype').value||'').toLowerCase();
  const isRechnung=dtype==='rechnung';

  // Pflichtfelder allgemein
  if(!$('idx-empf-typ').value)errors.push('Empfänger-Typ fehlt');
  if(!$('idx-empf-name').value.trim())errors.push('Empfänger-Name fehlt');
  if(!$('idx-absender').value.trim())errors.push('Absender fehlt');
  if(!$('idx-dok-datum').value.trim())errors.push('Dokumentdatum fehlt');
  if(!$('idx-eing-datum').value.trim())errors.push('Eingangsdatum fehlt');
  if(!$('idx-dtype').value)errors.push('Dokumentenart fehlt');

  // Pflichtfelder Rechnung
  if(isRechnung){
    if(!$('idx-belegnr').value.trim())errors.push('Belegnummer fehlt');
    if(!$('idx-betrag-netto').value.trim())errors.push('Rechnungsbetrag fehlt');
    if(!$('idx-zahlungsziel').value.trim())errors.push('Zahlungsziel fehlt');
  }

  const errDiv=$('idx-errors');
  const badge=$('idx-valid-badge');
  if(errors.length){
    errDiv.style.display='block';
    errDiv.innerHTML='<i class="ti ti-alert-triangle"></i> <b>Fehlende Pflichtfelder:</b> '+errors.join(' · ');
    badge.innerHTML='<span class="bdg berr"><i class="ti ti-x"></i> Unvollständig</span>';
    return false;
  } else {
    errDiv.style.display='none';
    badge.innerHTML='<span class="bdg bok"><i class="ti ti-check"></i> Vollständig</span>';
    return true;
  }
}

function saveIdxFields(){
  if(!validateIdxFields()){return;}
  // Daten in indexData übernehmen
  indexData.empf_typ=$('idx-empf-typ').value;
  indexData.empf_name=$('idx-empf-name').value.trim();
  indexData.absender=$('idx-absender').value.trim();
  indexData.dok_datum=$('idx-dok-datum').value.trim();
  indexData.eingangsdatum=$('idx-eing-datum').value.trim();
  indexData.dtype=$('idx-dtype').value;
  indexData.betreff=$('idx-betreff').value.trim();
  // Rechnungsfelder
  if(indexData.dtype==='Rechnung'){
    indexData.belegnummer=$('idx-belegnr').value.trim();
    indexData.betrag=$('idx-betrag-netto').value.trim();
    indexData.zahlungsziel=$('idx-zahlungsziel').value.trim();
    indexData.skonto=$('idx-skontosatz').value.trim();
    indexData.skonto_basis=$('idx-skonto-basis').value.trim();
    indexData.skontobetrag=$('idx-skontobetrag').value;
    indexData.zahlung_mit_skonto=$('idx-zahlung-skonto').value;
    indexData.skonto_frist=$('idx-skonto-frist').value.trim();
  }
  lg('ti-device-floppy','var(--gr)',`Indexfelder gespeichert: ${indexData.absender} · ${indexData.dtype}`);
  // Archiv aktualisieren
  if(nr)updateArchiveStatus(nr,'Indexiert');
}

function fillIdxFromAI(parsed){
  // KI-Ergebnisse in die Formularfelder übertragen
  if(parsed.absender)$('idx-absender').value=parsed.absender;
  if(parsed.datum)$('idx-dok-datum').value=parsed.datum;
  if(parsed.betreff)$('idx-betreff').value=parsed.betreff;
  // Eingangsdatum = heute
  $('idx-eing-datum').value=tod();
  // Dokumentenart
  const sel=$('idx-dtype');
  for(let i=0;i<sel.options.length;i++){
    if(sel.options[i].value===parsed.dtype){sel.selectedIndex=i;break;}
  }
  // Empfänger-Typ heuristisch
  if(parsed.absender){
    const a=parsed.absender.toLowerCase();
    if(/finanzamt|amt|behörde|gericht|bundesamt|senat|bezirk/.test(a))
      $('idx-empf-typ').value='Behörde';
    else if(/gmbh|ag|kg|inc|ltd|co\.|oHG/.test(a))
      $('idx-empf-typ').value='Lieferant';
  }
  // Rechnungsfelder
  if(parsed.betrag)$('idx-betrag-netto').value=parsed.betrag;
  if(parsed.zahlungsziel)$('idx-zahlungsziel').value=parsed.zahlungsziel;
  if(parsed.skonto)$('idx-skontosatz').value=parsed.skonto.replace('%','').trim();
  if(parsed.frist)$('idx-skonto-frist').value=parsed.frist;
  if(parsed.belegnummer)$('idx-belegnr').value=parsed.belegnummer;
...
// ═══ DATEIEN ════
let _addingFiles=false;
function addFiles(fl){
  if(_addingFiles)return; // Rekursionsschutz
  _addingFiles=true;
  try{
...
async function splitDoc(){
  // Veraltet - jetzt über openSplitModal()
  openSplitModal();
}
...
async function stampBrowser(rawB64,isImg,mime,einNr,datum,uhrzeit){
  ...
  if(isImg){
    ...
    doc=await PDFDocument.load(b64ToBytes(rawB64),{ignoreEncryption:true});
  }
  ...
  return bytesToB64(new Uint8Array(await doc.save()));
}
...
Antworte NUR mit dem JSON-Objekt, kein Text davor oder danach.`});
    ...
    // Archivieren mit Drive-Referenz
    addToArchive({nr,datum,uhrzeit,filename:f.name,
      drive_id:sd.ok&&sd.drive_id?sd.drive_id:'',
      ...parsed,tags:[...tags],status:'Neu'});
...
function addToArchive(entry){
  // drive_id aus Server-Antwort falls vorhanden
...
async function openArchiveDoc(idx){
  const e=archive[idx];
  if(!e)return;
  // Zum Workflow-Tab wechseln um Vorschau zu sehen
  showTab('wf',document.querySelector('.tab'));
  // Vorschau-Bereich zeigen
  $('prev-empty').style.display='none';
  $('prev-filename').textContent='· Laden...';
  $('prev-foot').innerHTML='<span style="color:var(--ac)"><i class="ti ti-loader"></i> Lade aus Google Drive...</span>';

  try{
    const r=await fetch('/drive/get',{method:'POST',headers:J,
      body:JSON.stringify({filename:e.filename||e.nr,drive_id:e.drive_id||''})});
    const d=await r.json();
    if(d.ok&&d.pdf_b64){
      // In Vorschau anzeigen
      $('prev-iframe').style.display='block';
      $('prev-img').style.display='none';
      $('prev-iframe').src='data:application/pdf;base64,'+d.pdf_b64;
      $('prev-filename').textContent='· '+d.filename;
      $('prev-toggle').style.display='flex';
      $('prev-dl').style.display='flex';
      $('prev-dl').onclick=()=>{
        const a=document.createElement('a');
      ...
      $('prev-foot').innerHTML=`
        <span style="color:var(--gr)"><i class="ti ti-brand-google-drive"></i> Aus Google Drive geladen</span>
        <span>${d.filename}</span>
        <span style="color:var(--tx2)">${e.datum||''} · ${e.absender||'–'}</span>`;
      lg('ti-brand-google-drive','var(--gr)',`Archiv: ${d.filename} aus Drive geladen ✓`);
    } else {
      $('prev-foot').innerHTML=`<span style="color:var(--re)"><i class="ti ti-alert-triangle"></i> ${d.error||'Nicht gefunden'}</span>`;
      $('prev-filename').textContent='· Nicht gefunden';
      lg('ti-alert-triangle','var(--am)',`Drive: ${d.error||'Datei nicht gefunden'}`);
    }
  }catch(ex){
    $('prev-foot').innerHTML=`<span style="color:var(--re)">Fehler: ${ex.message}</span>`;
  }
}
...
function renderArchive(items){
  ...
  // Index in Originalarchiv für Klick-Handler
  list.innerHTML=items.map(e=>{
    const idx=archive.indexOf(e);
    const cls=DOCTYPE_COLORS[e.dtype]||'dtype-sonstige';
    const hasDrive=e.filename||e.drive_id;
    return`<div onclick="openArchiveDoc(${idx})"
      style="border:.5px solid var(--bd);border-radius:var(--rs);padding:10px;
        margin-bottom:7px;background:var(--sf);cursor:${hasDrive?'pointer':'default'};
        transition:border-color .15s,box-shadow .15s"
      onmouseover="if(${hasDrive})this.style.borderColor='var(--ac)',this.style.boxShadow='0 1px 6px rgba(24,95,165,.15)'"
      onmouseout="this.style.borderColor='',this.style.boxShadow=''">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
          <span class="dtype ${cls}" style="font-size:10px;padding:1px 7px">${e.dtype||'–'}</span>
          <span style="font-size:11px;color:var(--tx2)">${e.datum||''} ${e.uhrzeit||''}</span>
          <span style="font-size:10px;color:var(--tx3)">[${e.nr||''}]</span>
          ${hasDrive?`<span style="font-size:10px;color:var(--ac)"><i class="ti ti-brand-google-drive"></i></span>`:''}
        </div>
        <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
          ${e.status?`<span style="font-size:10px;color:var(--gr);font-weight:500">${e.status}</span>`:''}
      ...
      <div style="font-size:12px;font-weight:500;margin-top:4px">${e.absender||e.empf_name||'–'}</div>
      ...
      ${e.filename?`<div style="font-size:10px;color:var(--tx3);margin-top:3px"><i class="ti ti-file-type-pdf"></i> ${e.filename}</div>`:''}
...
function searchArch(){
  ...
  renderArchive(archive.filter(e=>{
    ...
    if(q){
      // Suche in allen Feldern UND in Tags
      const tagMatch=(e.tags||[]).some(t=>t.toLowerCase().includes(q));
      const hay=JSON.stringify({
        absender:e.absender,betreff:e.betreff,betrag:e.betrag,
        belegnummer:e.belegnummer,nr:e.nr,empf_name:e.empf_name,
        filename:e.filename,status:e.status,dtype:e.dtype,datum:e.datum
      }).toLowerCase();
      if(!hay.includes(q)&&!tagMatch)return false;
    }
    return true;
  }));
}
...
async function execDec(){
  ...
  try{
    ...
    if(docQueue.length>0){
      ...
      renderQueue();
      lg('ti-player-skip-forward','var(--ac)',
        `Nächstes Dokument startet in 3 Sek. (${docQueue.length} verbleibend)`);
...
function resetAll(){
  ...
  $('stmp-prev').innerHTML='';$('donebox').innerHTML='';$('dtype-badge').innerHTML='';
  $('idx-box').style.display='none';
  // Pflichtfelder leeren
  ['idx-empf-typ','idx-empf-name','idx-absender','idx-dok-datum','idx-eing-datum',
   'idx-dtype','idx-betreff','idx-belegnr','idx-betrag-netto','idx-zahlungsziel',
   'idx-skontosatz','idx-skonto-basis','idx-skontobetrag','idx-zahlung-skonto',
   'idx-skonto-frist'].forEach(id=>{const e=$(id);if(e)e.value='';});
  $('rechnung-felder').style.display='none';
  $('idx-errors').style.display='none';
  if($('idx-valid-badge'))$('idx-valid-badge').innerHTML='';
  ...
  try{
    ...
    if(d.ok&&d.new_files?.length){
      ...
      if(d.new_files[0]?.pdf_b64){
        const blob=new Blob([b64ToBytes(d.new_files[0].pdf_b64)],{type:'application/pdf'});
    ...
    def do_POST(self):
        ...
        if self.path=='/config/save':
            ...
            try:
                ...
                cfg=load_config()
                drive_ok=False; drive_msg='Drive nicht konfiguriert'; drive_id=''
                if cfg.get('gdrive_in'):
                    fname=f"Posteingang_{nr}_{dat.replace('.','')}.pdf"
                    drive_ok,drive_msg=gdrive_upload_pdf(cfg,cfg['gdrive_in'],fname,stamped)
                    if drive_ok: drive_id=str(drive_msg); drive_msg='OK'
                self.send_json({'ok':True,'pdf_b64':stamped,'nr':nr,'datum':dat,'uhrzeit':uhr,
                    'drive_ok':drive_ok,'drive_msg':str(drive_msg),'drive_id':drive_id,
                    'filename':fname if cfg.get('gdrive_in') else ''})
            except Exception as e:
        ...
        elif self.path=='/drive/get':
            # Sucht und lädt eine Datei aus Drive anhand des Dateinamens
            cfg=load_config()
            filename=b.get('filename','')
            drive_id=b.get('drive_id','')  # direkte File-ID falls vorhanden
            try:
                token=gdrive_get_token(cfg)

                # Direkte File-ID bevorzugen
                if drive_id:
                    pdf_b64=gdrive_download_file(cfg,drive_id)
                    if pdf_b64:
                        self.send_json({'ok':True,'pdf_b64':pdf_b64,'filename':filename})
                        return

                # Suche nach Dateiname im Posteingang-Ordner
                folder_id=cfg.get('gdrive_in','').strip().split('?')[0]
                if not folder_id:
                    self.send_json({'ok':False,'error':'Posteingang-Ordner nicht konfiguriert'})
                    return

                # Suche mit originalem Namen
                ...
                # Falls nicht gefunden: auch im Postausgang suchen
                if not found:
                    folder_out=cfg.get('gdrive_out','').strip().split('?')[0]
                    if folder_out:
                        query2=f"name='{filename}' and '{folder_out}' in parents and trashed=false"
                        url2='https://www.googleapis.com/drive/v3/files?'+urllib.parse.urlencode({
                            'q':query2,'fields':'files(id,name,mimeType)','pageSize':'5'
                        })
                ...
                # Falls immer noch nicht gefunden: Teilstring-Suche
                if not found:
                    name_base=filename.split('.')[0][:30]  # erste 30 Zeichen des Namens
                    query3=f"name contains '{name_base}' and '{folder_id}' in parents and trashed=false"
                    url3='https://www.googleapis.com/drive/v3/files?'+urllib.parse.urlencode({
                        'q':query3,'fields':'files(id,name,mimeType)','pageSize':'5',
                        'orderBy':'createdTime desc'
                    })
                    ...
                    found=resp3.get('files',[])

                if not found:
                    self.send_json({'ok':False,'error':f'Datei nicht gefunden: {filename}'})
                    return

                file_id=found[0]['id']
                pdf_b64=gdrive_download_file(cfg,file_id)
                if pdf_b64:
                    self.send_json({'ok':True,'pdf_b64':pdf_b64,
                        'filename':found[0]['name'],'drive_id':file_id})
                else:
                    self.send_json({'ok':False,'error':'Download fehlgeschlagen'})
            except Exception as e:
                self.send_json({'ok':False,'error':str(e)})
